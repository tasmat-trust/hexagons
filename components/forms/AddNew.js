...
function AddNew(props) {
  ...
  async function handleForm(event) {
    ...
    if (event.target['textarea']) {
      formData.textarea = event.target['textarea'].value;
    }
    ...
    if (formResult.error) {
      ...
      const message = formResult.successMessage
      ...
      {!loading && (
        <>
          ...
          <FormControl margin="normal">
            <Button
              data-test-id={`add-new-${modelname}`}
              fullWidth
              type="submit"
              variant="contained"
              color="secondary"
...
function AddNewSummaryText({ summaryText, moduleId, successCallback }) {
  return (
    <AddNew
      successCallback={successCallback}
      moduleId={moduleId}
      initialTextAreaValue={summaryText}
      includeTextArea={true}
      actionName="Edit"
      updateHandler={updateSummaryText}
      modelname="summary"
    />
  );
}
